...
class TestCWN:
    ...
    def test_forward(self):
        ...
        adjacency_1 = torch.from_numpy(adjacency_1.todense()).float().to_sparse().to(device)
        incidence_1_t = torch.from_numpy(incidence_1_t.todense()).float().to_sparse().to(device)
        ...
        face_feature_matrix = torch.randn(len(cell_complex.cells),face_features).float().to(device)
        model = CWN(node_features,edge_features,face_features,16,1,2).to(device)
        with torch.no_grad():
            forward_pass = model(node_feature_matrix,edge_feature_matrix,face_feature_matrix,adjacency_1, incidence_2, incidence_1_t)
        assert torch.any(torch.isclose(input = forward_pass,other = torch.tensor([-0.7850]).to(device),rtol=1e-02))
