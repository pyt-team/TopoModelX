...
class TestCCXN:
    ...
    def test_forward(self):
        ...
        adjacency_0 = (
            torch.from_numpy(adjacency_0.todense()).float().to_sparse().to(device)
        )
        incidence_2_t = (
            torch.from_numpy(incidence_2_t.todense()).float().to_sparse().to(device)
        )
        node_feature_matrix = (
        ...
        with torch.no_grad():
            forward_pass = model(
                node_feature_matrix, edge_feature_matrix, adjacency_0, incidence_2_t
            )
        assert torch.any(
            torch.isclose(
                input=forward_pass, other=torch.tensor([-1.7816]).to(device), rtol=1e-02
            )
        )
