...
class TestCAN:
    ...
    def test_forward(self):
        ...
        with torch.no_grad():
                forward_pass = model(feature_nodes,feature_edges,\
                adjacency.float().to_sparse().to(device),lower_neighborhood,upper_neighborhood)
        assert torch.any(torch.isclose(input = forward_pass,other = torch.tensor([-0.0852, -0.3328]).to(device),rtol=1e-02))
    ...
    def test_forward_without_attn(self):
        ...
        with torch.no_grad():
                forward_pass = model(feature_nodes,feature_edges,\
                adjacency.float().to_sparse().to(device),lower_neighborhood,upper_neighborhood)
        assert torch.any(torch.isclose(input = forward_pass,other = torch.tensor([-0.0696,  0.0559]).to(device),rtol=1e-02))
