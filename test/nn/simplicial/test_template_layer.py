...
class TestTemplateLayer:
    ...
    def template_layer(self):
        ...
        return TemplateLayer(
            in_channels, intermediate_channels, out_channels, incidence_2
        )

    def test_forward(self, template_layer):
        """Test the forward pass of the template layer."""
        x = torch.randn(3, 10)
        output = template_layer.forward(x)
    ...
    def test_forward_with_invalid_input(self, template_layer):
        ...
        x = torch.randn(4, 10)
        try:
            template_layer.forward(x)
            assert False, "Expected ValueError to be raised."
        except ValueError as e:
            assert (
                str(e)
                == f"Shape of input face features does not have the correct number of faces {template_layer.incidence_2.shape[-1]}."
            )
