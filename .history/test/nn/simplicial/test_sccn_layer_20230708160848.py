...
class TestSCCNLayer:
    ...
    def test_forward(self):
        ...
        incidences = {"rank_1": 2 * torch.randint(0, 2, (n_rank_1_cells, n_rank_0_cells)).float() - 1}
                      
        adjacencies = {"rank_0": torch.eye(n_rank_0_cells).float(),
                       "rank_1": 2 * torch.randint(-1, 1, (n_rank_1_cells, n_rank_1_cells)).float() - 1}

        features = {"rank_0": 2 * torch.randn(n_rank_0_cells, channels) - 1, 
                    "rank_1": 2 * torch.randn(n_rank_1_cells, channels) - 1}
