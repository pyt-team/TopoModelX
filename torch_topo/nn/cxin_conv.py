...
class CXINConv( HigherOrderMessagePassing):
    ...
    def forward(self, x,a, aggregate_sign = True, aggregate_value= True):
        if self.message_type == "sender":
            output = self.mlp( self.propagate(x, a, aggregate_sign, aggregate_value) + (1 + self.epsilon ) * a.matmul(x)     )  # a sends x to a*x
        elif  self.message_type == "receiver": 
            # a must be symmetric
            output = self.mlp( self.propagate(x, a, aggregate_sign) + (1 + self.epsilon ) * x ) # a collects neighbors of x
        else:    
           raise RuntimeError(
               f" message type " f"'{self.message_type}' is not supported."
...
class CXINMerge(nn.Module):
    ...
    def forward(self, x1, x2, G1, G2):
        if G1.shape[0] != G2.shape[0]:
            raise ValueError(f"Input operators G1 and G2 must have the same target, got {G1.shape[0]} and {G2.shape[0]}")
...
class CXINMergeToTarget(HigherOrderMessagePassing):
    ...
    def __init__(
        ...
        super(CXINMergeToTarget,self).__init__(
            aggregate=aggregate)
    ...
    def forward(self, x_target, x_source, a, aggregate_sign = True ):
        ...
        if x_target.shape[-1] != x_source.shape[-1] : # make dimensions the same as the target
            support =  x_source @ self.weight
        else:
            support = x_source
        output = self.mlp( self.propagate( support , a, aggregate_sign) + (1 + self.epsilon ) * x_target)  # a collects neighbors of x
...
class CXINGeneral(nn.Module):
    ...
    def __init__(
        ...
        if merge_type not in ["conc","sum"]:
            raise RuntimeError(
                f"merge type must be in [conc,sum], got {merge_type}"
            )
        ...
        self.CXIN_list = nn.ModuleList([
         CXINMergeToTarget(i,
                           ...
                           aggregate,
                           kernel_initializer,
                           bias_initializer ) for i in in_ch_list])
    ...
    def forward(self, x_target, x_neighbor_list, G_list ):
        ...
        all_tensors = [self.CXIN_list[i](x_target,x_neighbor_list[i], G_list[i] ) for i in range(0,len(x_neighbor_list))]
        ...
        if self.merge_type == "conc":
            ...
            raise RuntimeError(
                " merge type must be in [conc,sum]"
            )
